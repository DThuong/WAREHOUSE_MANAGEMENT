...
const onChartClick = async (event: any, elements: any[], chart: any) => {
  ...
  let exactElements = chart.getElementsAtEventForMode(
    event.native ?? event,
    "nearest",
    { intersect: true },
    false,
  ...
  // If we clicked close but not directly intersecting a bar, fallback to nearest element with intersect: false
  if (!exactElements.length) {
    exactElements = chart.getElementsAtEventForMode(
      event.native ?? event,
      "nearest",
      { intersect: false },
      false,
  ...
  let datasetIndex = exactElements.length ? exactElements[0].datasetIndex : elements[0].datasetIndex;
  
  // Smart selection: if we used fallback (clicked blank space), prefer the dataset that actually has a non-zero value
  if (exactElements.length === 0 && elements.length > 1) {
    const nonZeroElement = elements.find(el => {
      const val = chartData.value?.datasets?.[el.datasetIndex]?.data[el.index];
      return val && Number(val) > 0;
    });
    if (nonZeroElement) {
      datasetIndex = nonZeroElement.datasetIndex;
    }
  }
